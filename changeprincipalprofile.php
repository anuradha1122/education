...
if($_SERVER["REQUEST_METHOD"] == "POST"){
  ...
  if(isset($_POST['submit'])){
    ...
    $emp_no       = $database -> data_existance($emp_no,'principal','emp_no');
    ...
    if(isset($_POST['email']) AND $_POST['email'] <> ''){
        ...
        if($email <> ''){
            $update       = $database -> update_one_column('principal', 'email', $email, 'emp_no', $emp_no);
            $activity .= ".23";
        }      
    }
    ...
    if(isset($_POST['password']) AND $_POST['password'] <> '' AND isset($_POST['cpassword']) AND $_POST['cpassword'] <> ''){
        ...
        if($password == $cpassword){
            ...
            $update   = $database -> update_one_column('principal', 'password', $password, 'emp_no', $emp_no);
            $activity .= ".24";
        } else{
  ...
  if(isset($_POST['cancel'])){
    ...
    $database -> headerto('changeprincipalprofile.php');
...
<table>
    <?php $database -> table_header('CHANGE PRINCIPAL PROFILE'); ?>
    ...
    <select name="app_state" style="width: 200px;"><option value="" selected>No Change</option><?php $st_query = $database ->table_search('appointment_state');
    ...
    <select name="position"> <option value="" selected>No Change</option><?php $position_query = $database ->table_search_by_element('principal','position','level');
    ...
    <select name="rank"><option value="" selected>No Change</option><?php $rank_query = $database ->table_search_by_element('principal','rank','level');
