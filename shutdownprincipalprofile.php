...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(isset($_POST['leave_detail']) AND $_POST['leave_detail'] <> '' AND isset($_POST['emp_no']) AND $_POST['emp_no'] <> '' AND isset($_POST['date']) AND $_POST['date'] <> ''){
        ...
        $update         = $database -> update_one_column('principal', 'leave_detail', $leave_detail, 'emp_no', $emp_no);
        $principal 		= $database -> table_by_id($emp_no,'principal','emp_no');
        $id 			= $principal['pr_id'];
        $update         = $database -> update_one_column('principal', 'active', '0', 'emp_no', $emp_no);
        $update         = $database -> update_leave_details('principal_appointment',$ip, $host_name, $date, $id, 'pr_id');
