...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $first_name   = strtoupper($database -> form_data_process('first_name'));
    ...
    $last_name    = strtoupper($database -> form_data_process('last_name'));
    ...
    $add_li1      = strtoupper($database -> form_data_process('add_li1'));
    $addli1_err   = $database -> field_missing($add_li1,'Address Line is Missing');

    $add_li2      = strtoupper($database -> form_data_process('add_li2'));
    $addli2_err   = $database -> field_missing($add_li2,'Address Line is Missing');

    $add_li3      = strtoupper($database -> form_data_process('add_li3'));
   ...
   $success  = $database -> insert_teacher_temp($emp_no,$first_name,$middle_name,$last_name,$sir_name,$birthday,$add_li1,$add_li2,$add_li3,$division,$gender,$race,$religion,$civil_status,$app_date,$app_state,$app_cata,$nic,$tel,$medium,$position,$ed_qu,$pr_qu,$rank,$function,$sub1,$sub2,$sub3,$sub4,$sub5,$email,$password,$creator,$ip,$host_name,$school);

    $school2       = $database -> form_data_process('school2');
    $school2       = $database -> table_by_id($school2,'school','name');

    $state2        = $database -> post_to_variabal($_POST['state2']);

    $f_date2       = $database -> form_data_process('f_date2');


    if($success <> '') {
        if(isset($school2) AND isset($f_date2)){

            $database -> insert_teacher_history($nic,$school2['school_no'],$state2,$f_date2);
        }

        $school3       = $database -> form_data_process('school3');
        $school3       = $database -> table_by_id($school3,'school','name');

        $state3        = $database -> post_to_variabal($_POST['state3']);

        $f_date3       = $database -> form_data_process('f_date3');

        if(isset($school3) AND isset($f_date3)){

            $database -> insert_teacher_history($nic,$school3['school_no'],$state3,$f_date3);
        }

        $school4       = $database -> form_data_process('school4');
        $school4       = $database -> table_by_id($school4,'school','name');

        $state4        = $database -> post_to_variabal($_POST['state4']);

        $f_date4       = $database -> form_data_process('f_date4');

        if(isset($school4) AND isset($f_date4)){

            $database -> insert_teacher_history($nic,$school4['school_no'],$state4,$f_date4);
        }

        $school5       = $database -> form_data_process('school5');
        $school5       = $database -> table_by_id($school5,'school','name');

        $state5        = $database -> post_to_variabal($_POST['state5']);

        $f_date5       = $database -> form_data_process('f_date5');

        if(isset($school5) AND isset($f_date5)){

            $database -> insert_teacher_history($nic,$school5['school_no'],$state5,$f_date5);
        }
    }
  ...
  if(isset($_POST['cancel'])){
    ...
    $database -> headerto('createteacherprofiletemp.php');
  ...
  <div class="row">
    <div class="col-sm-4 col-sm-offset-4 col-xs-offset-0 justify-content-center">
      ...
      <table class = "table table-hover">
        ...
        <td id="tableBox"><select name="rank"><?php $rank_query = $database ->table_search_decend('rank');
        ...
        <tr>
        <td id ="tableField"><h3>First School:</h3></td>
        <td class="search-box2" id="tableBox"></td>
        </tr>
        ...
        <tr>
        <td id ="tableField"><h3>Second School:</h3></td>
        <td id="tableBox"></td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <select name="state2"><?php $state_query = $database ->table_search('appointment_state');
        ...
        <td id="tableBox"><input type="date" name="f_date2" value=""></td>
        </tr>

        <tr>
        <td id ="tableField"><h3>Third School:</h3></td>
        <td id="tableBox"></td>
        </tr>

        <tr>
        <td id ="tableField">School:</td>
        <td class="search-box4" id="tableBox"><input type="text" name="school3" value="">
        ...
        <tr>
            <td id ="tableField">Appoint State:</td>
            <td>
                <select name="state3"><?php $state_query = $database ->table_search('appointment_state');
                while($state = mysqli_fetch_assoc($state_query)){ ?> 
                <option value="<?php echo $state['ap_id']; ?>"><?php echo $state['name']?></option> <?php }?> </select>
            </td>
        </tr>
        ...
        <tr>
        <td id ="tableField">Appointed Date:</td>
        <td id="tableBox"><input type="date" name="f_date3" value=""></td>
        </tr>

        <tr>
        <td id ="tableField"><h3>Fourth School:</h3></td>
        <td id="tableBox"></td>
        </tr>

        <tr>
        <td id ="tableField">School:</td>
        <td class="search-box5" id="tableBox"><input type="text" name="school4" value="">
        ...
        <tr>
            <td id ="tableField">Appoint State:</td>
            <td>
                <select name="state4"><?php $state_query = $database ->table_search('appointment_state');
                while($state = mysqli_fetch_assoc($state_query)){ ?> 
                <option value="<?php echo $state['ap_id']; ?>"><?php echo $state['name']?></option> <?php }?> </select>
            </td>
        </tr>
        ...
        <tr>
        <td id ="tableField">Appointed Date:</td>
        <td id="tableBox"><input type="date" name="f_date4" value=""></td>
        </tr>

        <tr>
        <td id ="tableField"><h3>Fifth School:</h3></td>
        <td id="tableBox"></td>
        </tr>

        <tr>
        <td id ="tableField">School:</td>
        <td class="search-box6" id="tableBox"><input type="text" name="school5" value="">
        ...
        <tr>
            <td id ="tableField">Appoint State:</td>
            <td>
                <select name="state5"><?php $state_query = $database ->table_search('appointment_state');
                while($state = mysqli_fetch_assoc($state_query)){ ?> 
                <option value="<?php echo $state['ap_id']; ?>"><?php echo $state['name']?></option> <?php }?> </select>
            </td>
        </tr>
        ...
        <tr>
        <td id ="tableField">Appointed Date:</td>
        <td id="tableBox"><input type="date" name="f_date5" value=""></td>
        </tr>
